...
def main():
    ...
    try:
        ...
        obstacle_transform = carla.Transform(carla.Location(x=50.0, y=0.0, z=0.2), carla.Rotation(yaw=0.0))
        obstacle = world.spawn_actor(obstacle_bp, obstacle_transform)
        obstacle.set_simulate_physics(True)
        print("✅ Obstacle spawned at 50m (moving at ~20 km/h)")
        ...
        while running:
            ...
            if obstacle is not None:
                try:
                    # Maintain ~20 km/h (5.56 m/s) — adjust throttle as needed
                    # In synchronous mode, must apply control EVERY tick
                    obstacle.apply_control(carla.VehicleControl(throttle=0.25, brake=0.0, steer=0.0))
                except Exception as e:
